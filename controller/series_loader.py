...
from view.series_loader_dialog import SeriesLoaderDialog
...
class SeriesLoader(object):
    ...
    def __init__(self):
        ...
        self._dialog = SeriesLoaderDialog()
        ##@var _file_dialog
        ...
        self._file_dialog.setAcceptMode(QFileDialog.AcceptOpen) #Sólo se acepta abrir fichero
    ...
    def _process(self):
        ...
        width = ui.widthValue.value() #ancho de la imagen, si raw
        height = ui.heightValue.value() #alto de la imagen, si raw
        first = ui.start.value() #inicio de los números de la serie
        last = ui.end.value() #fin de los números de la serie

        wt.change_info("Inicializando el cargador")

        ImageType = itk.Image[pt,dim] #tipo ITK de la imagen
        reader = itk.ImageSeriesReader[ImageType].New() #creamos el reader
        rawIO = None #imageIO para una imagen raw

        wt.change_value(10)
        if is_raw:
            wt.change_info("Inicializando el cargador RAW")
            rawIO = itk.RawImageIO[pt,2].New() #crear el rawIO (2D pues es para cad una de las imágenes)
        ...
        names_gen = itk.NumericSeriesFileNames.New() #crear el generador de imágenes
        names_gen.SetSeriesFormat( fileName ) #fijar el patrón de los nombres de ficheros
        names_gen.SetStartIndex( first ) #número inicial
        names_gen.SetEndIndex( last ) #número final
        names_gen.SetIncrementIndex( 1 ) #incremento de los números de las imágenes

        wt.change_info("Cargando imagen")
        wt.change_value(20)
        reader.SetFileNames(names_gen.GetFileNames()) #especificar el nombre del fichero a leer
        ...
        format = ""
        if name.rfind(".") != -1:
            format = name[name.rfind("."):] #obtener el formato
            name = name[:name.rfind(".")] #quitarle al nombre lo que está después del punto
        name = name.replace("%d","["+str(first)+":"+str(last)+"]")
    ...
    def run(self):
        try:
            res = self._dialog.exec_() #ejecutar el diálogo para obtner los datos de la imagen a cargar
